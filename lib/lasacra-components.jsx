/* La Sacra Corona — shared components + context. Exports to window. */
const { useState, useEffect, useRef, useContext, createContext } = React;

/* Context carries language, content bundle, and live tweak values. */
const LSCCtx = createContext(null);
const useLSC = () => useContext(LSCCtx);

/* ---- Crown seal emblem (placeholder for the studio's real mark) -------- */
function Emblem({ size = 56, ring = true, stroke = 1.4 }) {
  return (
...
/* ---- Logo lockup ------------------------------------------------------- */
function Logo({ variant = "inline", wordSize = 19, emblemSize = 38 }) {
  if (variant === "stack") {
    return (
...
function Reveal({ children, delay = 0, y = 22, as = "div", className = "", style = {} }) {
  ...
  useEffect(() => {
    ...
    // reveal anything in/near the viewport on first paint; scroll handles the rest
...
/* ---- Section header ---------------------------------------------------- */
function SectionHead({ idx, eyebrow, title, lede, action }) {
  return (
...
Object.assign(window, { LSCCtx, useLSC, Emblem, Logo, Stars, Reveal, SectionHead, TrustStrip });
