/* La Sacra Corona — Galería + Reservas (2 variants) + Localización + Footer + floats */
const { useState: useStateB, useEffect: useEffectB, useMemo: useMemoB } = React;

/* =========================================================================
   GALERÍA — filter tabs + masonry + lightbox
   ========================================================================= */
function Galeria() {
  const { c, data } = useLSC();
  const [filter, setFilter] = useStateB("all");
  const [box, setBox] = useStateB(null); // index in filtered list
...
/* =========================================================================
   RESERVAS — shared booking logic + 2 layouts
   ========================================================================= */
...
function useBooking() {
  ...
  const [status, setStatus] = useStateB("idle"); // idle | sending | done
...
/* =========================================================================
   TESTIMONIOS — client reviews grid
   ========================================================================= */
...
/* =========================================================================
   LOCALIZACIÓN — two studios + dark stylized map
   ========================================================================= */
function MapDark({ seed }) {
  // abstract dark map tile — stylized streets + blood pin
  const lines = seed === 0
...
Object.assign(window, { Galeria, Testimonios, Reservas, Localizacion, Footer, WhatsAppFloat });
