/* La Sacra Corona — motion layer.
   Preloader, Lenis smooth scroll, scroll progress, custom cursor, magnetic
   CTAs, light parallax. Called once by the React app after mount:
     window.LSCMotion.init()
   Everything degrades gracefully: no Lenis -> native scroll; reduced-motion
   or touch -> static. */
(function () {
  let inited = false;
  ...
  function runLoader(reduce) {
    ...
    const finish = () => {
      ...
      // remove loader from the tree after it slides away (so it can't trap clicks)
      setTimeout(() => { if (loader) loader.style.display = "none"; }, 1100);
      // settle scroll positions / triggers
  ...
  /* ---- Smooth scroll (self-contained inertia, desktop only) --------- */
  let SMOOTH = false;
  function setupSmoothScroll() {
    ...
    // when scroll is driven by anything else (keyboard, bar, anchor jump), resync
