/* La Sacra Corona — hero atmosphere.
   Slow-drifting smoky radial fields + a faint blood ember on a dark ground.
   Deliberately low-contrast: it reads as depth, not decoration.
   window.LSCAtmosphere.mount(canvas) -> { destroy() }  */
(function () {
  function mount(canvas) {
    ...
    // smoky blobs: warm charcoals + one ember
    const blobs = [
      ...
      { x: 0.60, y: 0.80, r: 0.46, col: [86, 18, 16],  amp: 0.05, sp: 0.07, ph: 4.4,  a: 0.30 }, // ember (blood)
    ];
    ...
    function emberAccent() {
      // tweak panel can switch accent off -> dim the ember
    ...
    function draw() {
      ...
      // top + bottom fade so the canvas melts into the page
